...
{ //Xmister
	...
	memset(b, 0, sizeof(struct i2c_board_info));
	

	strncpy(c->name, max8698_driver.driver.name, I2C_NAME_SIZE);
	strncpy(b->type, max8698_driver.driver.name, I2C_NAME_SIZE);
	c->addr = addr;
	b->addr = addr;
	c->adapter = adap;
	c->driver = &max8698_driver;
	b->platform_data = &max8698_driver;

	/*if ((ret = i2c_attach_client(c)))
		goto error;
	*/
	if( (c = i2c_new_device(adap,b) ) == NULL)
        	goto error;
...
static struct i2c_driver max8698_driver = {
	...
	.shutdown = max8698_detach_client,
	.command = max8698_command,
	.probe		= max8698_probe,
	.address_list	= max8698_normal_i2c,
};
