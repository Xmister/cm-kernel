...
//Xmister
struct i2c_device_id x_dev_id =
{ "qt5480_i2c", QT5480_I2C_ADDR };
...
static struct i2c_driver g_qt5480_i2c_driver = {
    ...
    .address_list = g_qt5480_i2c_normal,
};
...
static int qt5480_attach(struct i2c_adapter *aAdapter, int aAddress, int aKind)
    ...
    b->platform_data = &g_qt5480_i2c_driver;

    /*
    if((ret = i2c_attach_client(c)) < 0)
        goto error;
    */
    if( (c = i2c_new_device(aAdapter,b) ) == NULL)
        goto error;
...
static int i2c_attach_adapter(struct i2c_adapter *aAdapter)
    ...
    ENTER_FUNC;
    //Xmister
    return i2c_add_adapter(aAdapter);
    }
...
int __init qt5480_init(void)
    ...
    //Xmister
    g_ts_dev = device_create(NULL, NULL, 0, NULL, "ts");
    if (IS_ERR(g_ts_dev))
        {
